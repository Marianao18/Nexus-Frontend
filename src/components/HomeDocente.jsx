...
export default function HomeDocente() {
    ...
    const handlePassSave = async () => {
        ...
        if (!pass.actual || !pass.nueva || !pass.confirmar) {
            setPassMsg('err:Todos los campos son obligatorios.');
            return;
        }
        if (pass.nueva !== pass.confirmar) {
            setPassMsg('err:Las nuevas contraseñas no coinciden.');
            return;
        }
        try {
            const token = localStorage.getItem('access_token');
            const res = await axios.post('http://localhost:8000/api/usuarios/cambiar-password-perfil/', 
            ...
            const errorServer = err.response?.data?.error || "Error al conectar con el servidor.";
            setPassMsg(`err:${errorServer}`);
        }
    };
        ...
        <div className="nd-view nd-fade-in">
            ...
            <div className="nd-profile-card">
                ...
                <div className="nd-profile-readonly">
                    <p className="nd-readonly-note"> Los siguientes campos no son editables — fueron definidos al crear tu cuenta.</p>
